...
public class WebPageReader implements Runnable {
    ...
    public WebPageIndexed altWords;
    ...
    private static String getContents (URL url, String encoding) {
        ...
        try {
            scanner =
                new Scanner (new
                             BufferedInputStream
                                (url.openConnection().getInputStream()), encoding);
        } catch (IOException e) {
            return "";
        }
    ...
    // Function overload collects values to HashMap instead of HashSet
    // This is a non intrusive way of adding my test functionality
    private void collectWords (String contents) {
        Scanner sc = new Scanner (contents);
        sc.useDelimiter(WORD_DELIMITER);
        int index = 0;
        while (sc.hasNext ()) {
            String word = sc.next ();
            try {
                this.altWords.words.get(word).add(index);
            } catch (NullPointerException n){
                ArrayList<Integer> temp = new ArrayList<>();
                temp.add(index);
                this.altWords.words.put(word, temp);
                index++;
            }
        }
    }
    ...
    private static void basicTest () {
        ...
        WebPageReader wp = new WebPageReader ("https://en.wikipedia.org/wiki/Main_Page");
        wp.run ();
        System.out.println(wp.altWords.linkName);
